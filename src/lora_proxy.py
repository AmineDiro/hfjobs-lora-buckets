"""Prefix-aware, adapter-broadcasting reverse proxy in front of N vLLM servers, for AsyncGRPO LoRA on HF Jobs.
...
class Config:
    def __init__(self, env=os.environ):
        self.upstreams = [
            u.strip().rstrip("/") for u in env["UPSTREAM_URLS"].split(",") if u.strip()
        ]
...
class PrefixRouter:
    ...
    def block_hashes(self, model: str, prompt) -> list[bytes]:
        ...
        for i in range(0, len(ids) - size + 1, size):
            chunk = ids[i:i + size]
            prev = hashlib.blake2b(prev + (bytes(chunk) if isinstance(chunk, bytes) else json.dumps(chunk).encode()),
...
def make_app(cfg: Config) -> web.Application:
    ...
    async def send(
        up: Upstream, method: str, path_qs: str, headers: dict, body: bytes | None
    ):
    ...
    async def handle(request: web.Request) -> web.StreamResponse:
        ...
        if path in ROUTED and request.method == "POST":
            ...
            if not isinstance(prompt, (str, list)):
                prompt = (
                    json.dumps(prompt, sort_keys=True) if prompt is not None else ""
                )
        ...
        if path in BROADCAST:
            results = await asyncio.gather(
                *(send(u, request.method, request.path_qs, headers, body) for u in ups)
            )
            if (
                path == "/v1/unload_lora_adapter"
            ):  # missing names are not an error, matching vLLM's own semantics
                return web.Response(
                    status=200, body=results[0][2], content_type="application/json"
                )
            bad = [(u, r) for u, r in zip(ups, results) if r[0] != 200]
            if bad:
                u, r = bad[0]
                return web.Response(
                    status=r[0],
        ...
        if path == "/proxy/stats":
            return web.json_response(
                {
                    "upstreams": [
                        {"url": u.url, "inflight": u.inflight, "routed": u.routed}
                        for u in ups
                    ],
    ...
    async def load_lora(
        request: web.Request, headers: dict, body: bytes
    ) -> web.Response:
        ...
        for u, st, text, attempts in results:
            print(
        ...
        if failed:
            ...
            await asyncio.gather(
                *(
                    send(u, "POST", "/v1/unload_lora_adapter", headers, unload)
                    for u, *_ in ok
                )
            )
        ...
        return web.Response(
            status=200, body=results[0][2].encode(), content_type="application/json"
        )
